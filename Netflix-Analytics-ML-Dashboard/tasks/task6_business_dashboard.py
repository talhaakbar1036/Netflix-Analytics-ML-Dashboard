...
FIXED VERSION:
- Uses FAIR model (without leaky features) for classification
- Better business recommendations
- Added data quality insights

Workflow:
 Step 1: Perform complete data analysis.
 Step 2: Develop predictive models (fair version).
...
def run():
    ...
    top_genres = df["listed_in"].apply(lambda x: [g.strip() for g in str(x).split(",")]).explode().value_counts().head(5)
    ...
    missing_country_pct = (df["country"] == "Unknown").mean() * 100
    ...
    print(f"Data quality: {missing_director_pct:.1f}% missing director, {missing_country_pct:.1f}% missing country")

    # ------------------------------------------------------------------
    # Step 2: Develop predictive models (FAIR version - no leaky features)
    # ------------------------------------------------------------------
    ...
    # 2b. Classification model (FAIR version - no duration or director_known)
    print("\nTraining FAIR classification model (no leaky features)...")
    ...
    # FAIR features only: genre, country, rating, release_year
    ...
    print(f"Fair content-type classifier accuracy: {clf_acc:.3f}")
    print("(Uses only genre, country, rating, release_year - no leaky features)")
    ...
    # Feature importance for FAIR classifier
    fair_features = ["genre", "country", "rating", "release_year"]
    importances = pd.Series(clf.feature_importances_, index=fair_features).sort_values(ascending=False)
    sns.barplot(x=importances.values, y=importances.index, ax=axes[1, 2], color="#E50914")
    axes[1, 2].set_title("Fair Classifier - Feature Importance\n(No Leaky Features)")
    ...
    recommendations = [
        ...
        f"indicating a mature-audience skew; family-friendly content is under-represented.",
        
        f"5. Data quality: {missing_director_pct:.0f}% of titles are missing director metadata. "
        f"Improving metadata completeness would improve recommendation-engine accuracy.",
        
        f"6. ML Model: Fair classifier (no leaky features) achieves {clf_acc:.1%} accuracy. "
        f"This is a realistic performance estimate for production use.",
    ]
    ...
    with open(report_path, "w") as f:
        ...
        f.write("FIXED VERSION: Uses FAIR ML model (no data leakage)\n")
        f.write("=" * 70 + "\n\n")
        ...
        f.write(f"Top 5 genres:\n{top_genres.to_string()}\n\n")

        f.write("3. PREDICTIVE MODELS (FAIR - NO LEAKAGE)\n" + "-" * 40 + "\n")
        f.write(f"Growth forecast (2022-2025):\n{forecast_df.to_string(index=False)}\n\n")
        f.write(f"Content-type classifier (fair) accuracy: {clf_acc:.3f}\n")
        f.write("Features used: genre, country, rating, release_year\n")
        f.write("Excluded: duration_value, director_known (leaky features)\n\n")

        f.write("4. DATA QUALITY ISSUES\n" + "-" * 40 + "\n")
        f.write(f"Missing director: {missing_director_pct:.1f}%\n")
        f.write(f"Missing country: {missing_country_pct:.1f}%\n")
        f.write("These gaps affect recommendation quality and should be addressed.\n\n")

        f.write("5. BUSINESS RECOMMENDATIONS\n" + "-" * 40 + "\n")
        for r in recommendations:
            f.write(r + "\n\n")

        f.write("6. SUPPORTING VISUALS\n" + "-" * 40 + "\n")
